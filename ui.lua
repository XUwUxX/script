-- KHub UI v5.1 – Đầy đủ, ổn định, hỗ trợ Slider with textbox
...
local C = {
    WinBg   = Color3.fromRGB(18,18,22), TopBg   = Color3.fromRGB(22,22,28),
    SideBg  = Color3.fromRGB(22,23,28), ContBg  = Color3.fromRGB(20,22,26),
    Stroke  = Color3.fromRGB(42,42,56), Text    = Color3.fromRGB(225,225,240),
    SecBg   = Color3.fromRGB(28,30,36), BtnBg   = Color3.fromRGB(38,40,50),
    BtnHov  = Color3.fromRGB(58,62,82), BtnPrs  = Color3.fromRGB(32,34,68),
...
local function CreateConsole(parent)
    local con = Frame{ Name="Console", Parent=parent, Size=UDim2.new(1,-8,0,24), BackgroundColor3=C.LogBg, LayoutOrder=999 }
    local logs, open = {}, false
    local scroll = Frame{ Name="Scroll", Parent=con, Visible=false, Size=UDim2.new(1,-4,0,100), Position=UDim2.new(0,2,0,26), BackgroundColor3=C.LogBg, ClipsDescendants=true, ZIndex=5 }
...
function KHub:MakeWindow(opt)
    ...
    UI.screenGui = gui
    local win = Frame{ Name="Main", Parent=gui, AnchorPoint=Vector2.new(0.5,0.5), Position=UDim2.fromScale(0.5,0.5), Size=UDim2.new(0,560,0,390), BackgroundColor3=C.WinBg }
    UI.window = win
    ...
    topBtn("–", -62, function()
        ...
        rest.InputChanged:Connect(function(i)
            if drag and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
                ...
                rest.Position = UDim2.new(pos.X.Scale, pos.X.Offset+d.X, pos.Y.Scale, pos.Y.Offset+d.Y)
            end
        end)
        rest.InputBegan:Connect(function(i)
            if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
    ...
    local function onDragEnd(input) dragging = false end
    ...
    for _, btn in ipairs(topBar:GetChildren()) do
        if btn:IsA("Frame") and btn.Name ~= "Top" then
            btn.InputBegan:Connect(function() dragging = false end)
        end
    end
    ...
    sf.Size, sf.Position = UDim2.new(1,-2,1,-2), UDim2.new(0,1,0,1)
    ...
    KHub:Log("Core", "Kéo thả & scroll OK", Color3.fromRGB(173,216,230))
    ...
    function Window:MakeTab(tabOpt)
        local btn = Frame{ Name=tabOpt.Name.."Tab", Parent=sf, Size=UDim2.new(1,-8,0,32), LayoutOrder=#tabs+2, BackgroundColor3=C.TabIn }
        ...
        local page = Frame{ Name=tabOpt.Name.."Page", Parent=contentArea, Size=UDim2.new(1,0,1,0), BackgroundColor3=C.ContBg, Visible=false }
        ...
        function Tab:AddSection(secOpt)
            local sec = Frame{ Name=secOpt.Name.."Sec", Parent=pageScroll, Size=UDim2.new(1,-16,0,0), BackgroundColor3=C.SecBg, LayoutOrder=#tabs+1 }
            ...
            function Section:AddButton(opt)
                local btn = Frame{ Name=opt.Name.."Btn", Parent=sec, Size=UDim2.new(0,120,0,30), BackgroundColor3=C.BtnBg, LayoutOrder=order }
                order += 1
                local lbl = Instance.new("TextLabel", btn)
            ...
            function Section:AddToggle(opt)
                ...
                order += 1
                local bg = Frame{ Name="Tog", Parent=container, Size=UDim2.new(0,44,0,26), Position=UDim2.new(0,0,0.5,-13), BackgroundColor3=opt.Default and C.TogOn or C.TogOff }
                local knob = Frame{ Name="Knob", Parent=bg, Size=UDim2.new(0,22,0,22), Position=UDim2.new(0,1,0,1), BackgroundColor3=Color3.fromRGB(255,255,255) }
            ...
            function Section:AddSlider(opt)
                ...
                if opt.WithTextbox then
                    tb = Instance.new("TextBox", container)
                    tb.Size = UDim2.new(0,36,0,20)
                    tb.Position = UDim2.new(0,w+6,0,10)
                    tb.Text = tostring(val)
                    tb.BackgroundColor3 = C.SecBg
                ...
                local function set(v)
                    ...
                    if tb then tb.Text = tostring(math.floor(v)) end
            ...
            function Section:AddDropdown(opt)
                ...
                container.BackgroundTransparency, container.Size, container.LayoutOrder = 1, UDim2.new(1,0,0,30), order
                order += 1
                local main = Frame{ Name="Drop", Parent=container, Size=UDim2.new(0,140,0,26), Position=UDim2.new(0,0,0,0), BackgroundColor3=C.BtnBg }
                ...
                local menu = Frame{ Name="Menu", Parent=container, Size=UDim2.new(0,140,0,0), Position=UDim2.new(0,0,1,2), BackgroundColor3=C.SecBg, Visible=false, ZIndex=10, ClipsDescendants=true }
            ...
            function Section:AddParagraph(text)
                local para = Frame{ Name="Para", Parent=sec, BackgroundColor3=C.SecBg, Size=UDim2.new(1,-10,0,0), LayoutOrder=order, AutomaticSize=Enum.AutomaticSize.Y }
                order += 1
    ...
    function Window:Destroy() KHub:Destroy() end
    return Window
end
    ...
    if pcall(function() return game:GetService("StarterGui"):SetCore("SendNotification",{}) end) then
        pcall(function()
            ...
            if icon then
                ...
                icon = "rbxassetid://11836181348"
            end
            game:GetService("StarterGui"):SetCore("SendNotification",{
                Title = opt.Name or "KHub",
                Text = opt.Content or "",
